      ...
      <div ref={ref} className="max-w-[68rem] mx-auto px-[60px] max-lg:px-6 max-md:px-4 fade-in-view">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div>
            ...
            <h2 className="text-stone font-bold leading-[1.3] tracking-tight mb-5" style={{ fontSize: 'clamp(2.2rem, 4vw, 2.6rem)' }}>Start Your Insurance Journey Today</h2>
            <p className="text-cream font-light mb-8" style={{ lineHeight: '22px', maxWidth: '500px' }}>Tell us about your business and our commercial insurance specialists will reach out with tailored coverage options. No obligation — just expert guidance from a team that understands your industry.</p>
            <button onClick={() => window.dispatchEvent(new Event('open-quote-modal'))} className="inline-flex items-center justify-center border-2 border-gold bg-gold text-brand text-center uppercase tracking-[0.16em] rounded-[2rem] font-semibold hover:border-stone hover:bg-stone hover:text-brand cursor-pointer" style={{ padding: '0.8rem 2.5rem 0.7rem', fontSize: '0.75rem', lineHeight: '2', transition: 'all 0.24s' }}>
          ...
          <div className="relative overflow-hidden rounded-[2rem]" style={{ minHeight: '400px' }}>
            <img src="https://images.unsplash.com/photo-1556761175-5973dc0f32e7?auto=format&fit=crop&w=800&q=80" alt="Business insurance specialist helping business owners find coverage" width={800} height={600} loading="lazy" className="absolute inset-0 w-full h-full object-cover" style={{ filter: 'grayscale(100%)' }} />
            ...
            <div className="relative h-full flex flex-col items-center justify-center text-center p-10" style={{ minHeight: '400px' }}>
              <h3 className="text-stone font-bold mb-4" style={{ fontSize: '1.8rem', lineHeight: '1.3' }}>Ready to Protect Your Business?</h3>
              <p className="text-cream/80 font-light mb-8 max-w-sm" style={{ fontSize: '0.95rem', lineHeight: '1.5' }}>Our specialists understand commercial operations inside and out. Get coverage tailored to the way you run your business.</p>
              <button onClick={() => window.dispatchEvent(new Event('open-quote-modal'))} className="inline-flex items-center justify-center border-2 border-stone text-stone bg-transparent text-center uppercase tracking-[0.16em] rounded-[2rem] font-semibold hover:bg-stone hover:text-brand cursor-pointer" style={{ padding: '0.8rem 2rem 0.7rem', fontSize: '0.75rem', lineHeight: '2', transition: 'all 0.24s' }}>
                Start Your Quote
              </button>
            </div>
          </div>

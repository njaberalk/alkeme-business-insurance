...
const coverageItems = [
  ...
  { label: 'Business Interruption', href: '/coverage/business-interruption/' },
  ...
  { label: 'Employment Practices', href: '/coverage/employment-practices-liability/' },
];
...
const resourceItems = [
  ...
  { label: 'Business Insurance Cost', href: '/resources/business-insurance-cost/' },
  { label: 'General Liability Guide', href: '/resources/general-liability-guide/' },
  { label: 'Workers\' Comp Guide', href: '/resources/workers-comp-guide/' },
  { label: 'Cyber Insurance Guide', href: '/resources/cyber-insurance-guide/' },
  { label: 'Insurance Glossary', href: '/resources/business-insurance-glossary/' },
  { label: '— Insights —', href: null, divider: true },
  { label: 'Market Trends 2026', href: '/blog/business-insurance-trends-2026/' },
  { label: 'Common Mistakes', href: '/blog/common-business-insurance-mistakes/' },
  { label: 'Cyber Threats Guide', href: '/blog/cyber-threats-small-business/' },
];
...
function Dropdown({ label, items, isOpen, onToggle, onClose }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={onToggle} onMouseEnter={onToggle} className="text-brand hover:text-blue-dark uppercase tracking-[0.16em] px-5 py-2 font-bold flex items-center gap-1" style={{ fontSize: '0.75rem', transition: 'all 0.2s' }}>
        {label}
        <svg className="w-3 h-3" style={{ transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 bg-stone rounded-[1rem] border border-ash py-3 min-w-[240px]" style={{ boxShadow: '0 2px 5px rgba(0,0,0,0.2)', animation: 'fadeIn 0.15s ease' }} onMouseLeave={onClose}>
          {items.map((item) => (
            item.divider ? (
              <div key={item.label} className="px-5 pt-3 pb-1 text-blue-dark/50 uppercase tracking-[0.12em] font-bold border-t border-ash first:border-0 first:pt-0 mt-1 first:mt-0" style={{ fontSize: '0.6rem' }}>{item.label.replace(/—/g, '').trim()}</div>
            ) : (
              <Link key={item.label} href={item.href} onClick={onClose} className="block px-5 py-2 text-brand hover:text-blue-dark hover:bg-ash/30 uppercase tracking-[0.12em] font-semibold no-underline" style={{ fontSize: '0.7rem', transition: 'all 0.2s' }}>{item.label}</Link>
            )
          ))}
        </div>
      )}
...
export default function Header() {
  ...
  const isHome = pathname === '/business-insurance' || pathname === '/business-insurance/' || pathname === '/';
        ...
        <div className="max-w-[68rem] mx-auto px-[60px] max-lg:px-6 max-md:px-4">
          <div className="flex items-center justify-between">
            <a href="https://alkemeins.com" className="shrink-0 no-underline">
              <img src="https://cdn.prod.website-files.com/686d7240ee8f0fdd31648b80/68ae3aa829e8b536f770d83d_ALKEME_Logo_Color.png" alt="ALKEME Insurance Services" className="h-auto" style={{ width: '9rem' }} />
            </a>
            <nav className="hidden lg:flex items-center gap-0" role="navigation" aria-label="Main navigation">
              <Dropdown label="Coverage" items={coverageItems} isOpen={openDropdown === 'coverage'} onToggle={() => setOpenDropdown(openDropdown === 'coverage' ? null : 'coverage')} onClose={() => setOpenDropdown(null)} />
        ...
        <div className="lg:hidden bg-stone overflow-hidden" style={{ maxHeight: mobileOpen ? '90vh' : '0', transition: 'max-height 0.4s ease', borderTop: mobileOpen ? '1px solid #e3e3d8' : 'none' }}>
          <div className="px-6 py-6 space-y-1 max-h-[80vh] overflow-y-auto">
            ...
            {coverageItems.map((item) => (
              <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)} className="block text-brand hover:text-blue-dark uppercase tracking-[0.12em] font-semibold py-2 no-underline" style={{ fontSize: '0.7rem', transition: 'all 0.2s' }}>{item.label}</Link>
            ))}
            ...
            <div className="border-t border-ash pt-4 mt-4 space-y-3">
              ...
              <Link href="/resources/business-insurance-cost/" onClick={() => setMobileOpen(false)} className="block text-brand hover:text-blue-dark uppercase tracking-[0.12em] font-semibold py-2 no-underline" style={{ fontSize: '0.7rem' }}>Business Insurance Cost Guide</Link>
              <Link href="/resources/general-liability-guide/" onClick={() => setMobileOpen(false)} className="block text-brand hover:text-blue-dark uppercase tracking-[0.12em] font-semibold py-2 no-underline" style={{ fontSize: '0.7rem' }}>General Liability Guide</Link>
              <Link href="/blog/business-insurance-trends-2026/" onClick={() => setMobileOpen(false)} className="block text-brand hover:text-blue-dark uppercase tracking-[0.12em] font-semibold py-2 no-underline" style={{ fontSize: '0.7rem' }}>Market Trends 2026</Link>

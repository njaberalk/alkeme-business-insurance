  ...
  return (
    <section className="relative overflow-hidden bg-brand" style={{ paddingTop: '12rem', paddingBottom: '6rem' }}>
      <div className="absolute inset-0">
        <img src="https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=1920&q=80" alt="Modern commercial building for business insurance" width={1920} height={600} className="absolute inset-0 w-full h-full object-cover" style={{ opacity: 0.35, objectPosition: '50% 30%', filter: 'grayscale(100%)' }} />
      ...
      <div className="relative max-w-[68rem] mx-auto px-[60px] max-lg:px-6 max-md:px-4">
        <p className="text-cream font-light max-w-[600px]" style={{ fontSize: '1.1rem', lineHeight: '1.6', opacity: loaded ? 1 : 0, transform: loaded ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s ease' }}>{subheading}</p>
        <p className="text-blue uppercase tracking-[0.15em] font-bold mb-4 mt-4" style={{ fontSize: '0.85rem', opacity: loaded ? 1 : 0, transform: loaded ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s ease 0.1s' }}>{label}</p>
        <h1 className="text-stone font-extrabold tracking-tight mb-4" style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', lineHeight: '1.1', opacity: loaded ? 1 : 0, transform: loaded ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.7s ease 0.15s' }}>{heading}</h1>
